...
'//
'// Example:
'// szFilename = DialogFile(Me.hWnd, 1, "Open", "MyFileName.doc", "Documents" & Chr(0) & "*.doc" & Chr(0) & "All files" & Chr(0) & "*.*", gstrAppPath, "doc")
'//
...
Public Function DialogFile(hWnd As Long, wMode As Integer, szDialogTitle As String, szFilename As String, szFilter As String, szDefDir As String, szDefExt As String) As String
    ...
    OFN.lpstrFile = szFilename & String$(1000 - Len(szFilename), 0)
    OFN.nMaxFile = 1000
    OFN.lpstrFileTitle = String$(1000, 0)
    OFN.nMaxFileTitle = 1000
    OFN.lpstrFilter = Replace(szFilter, "|", vbNullChar)
    OFN.nFilterIndex = 1
    ...
    ElseIf wMode = 2 Then
        OFN.Flags = OFN_HIDEREADONLY Or OFN_PATHMUSTEXIST Or OFN_FILEMUSTEXIST Or OFN_ALLOWMULTISELECT Or OFN_EXPLORER Or OFN_LONGNAMES
        X = GetOpenFileName(OFN)
    ElseIf wMode = 3 Then
